...
func (fe *frontendServer) viewCartHandler(w http.ResponseWriter, r *http.Request) {
	...
	// custom span starts here
	ctx, calculateSpan := otel.Tracer("frontend").Start(r.Context(), "calculate-shipping-cost")
	shippingCost, err := fe.getShippingQuote(ctx, cart, currentCurrency(r))
	if err != nil {
		calculateSpan.RecordError(err)
		calculateSpan.End()
		renderHTTPError(log, r, w, errors.Wrap(err, "failed to get shipping quote"), http.StatusInternalServerError)
		return
	}
	calculateSpan.SetAttributes(attribute.Int("cart.items", len(cart)))
	calculateSpan.End()
...
func (fe *frontendServer) placeOrderHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Create a business-logic span to record the order success details
	_, checkoutSpan := otel.Tracer("frontend").Start(r.Context(), "complete-checkout")
	...
	checkoutSpan.SetAttributes(
		attribute.String("user.id", sessionID(r)),
		attribute.Int("order.items_count", len(order.GetOrder().GetItems())),
		attribute.Float64("order.total", float64(totalPaid.GetUnits())+float64(totalPaid.GetNanos())/1e9),
	)

	if checkoutCounter != nil {
		checkoutCounter.Add(r.Context(), 1, metric.WithAttributes(attribute.String("currency", currentCurrency(r))))
	}
